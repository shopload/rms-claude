...
1. [RMSメンバーズステーション](https://mstation.rms.rakuten.co.jp/)にショップ管理者権限でログインする。
2. `拡張機能` → `WEB API連携`（システム連携／WEB Service利用申請）メニューを開く。
3. 使用するスキルに合わせて、以下のWEB Serviceの利用を申請・許可する（未使用のAPIは許可しない＝最小権限の原則）。
...
4. 許可が反映されたら、同じ `WEB API連携` 画面（ライセンス管理）で**サービスシークレット**と**ライセンスキー**を確認する。
