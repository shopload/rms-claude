...
3. `利用設定` → `アプリ一覧` を開き、対象の店舗様のアプリを選択して `詳細` に入る。
4. `利用機能一覧` / `利用機能編集` から、使用するスキルに合わせて以下のWEB Serviceを「利用中」に変更する（未使用のAPIは利用中にしない＝最小権限の原則）。
...
5. 内容を確認・登録すると**サービスシークレット**が発行される。**ライセンスキー**も同じ `詳細` 画面で確認できる。
